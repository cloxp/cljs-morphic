...
(defn applicative [morph]
  ...
  (fn [new-props & new-submorphs]
    (let [submorphs (apply concat (map (fn [m]
                                           (if (morph? m)
                                             [m]
                                             m)) new-submorphs))
...
(defn apply-to-morph [function m]
  ...
  (let [[morph props & submorphs] m]
    (function (applicative morph) 
            props 
            submorphs)))

(defn destructure-morph [morph]
  [(applicative morph) 
   (:props morph) 
   (:submorphs morph)])

(defn where [morphs test-fn]
  (destructure-morph (first (filter #(apply-to-morph test-fn %) morphs))))

(defn some-morph [morphs test-fn]
  (some #(apply-to-morph test-fn %) morphs))
